...
Prompt 76 validated dispatch review-state behavior. Local QA confirmed
incomplete Ready rejection, complete Ready, Reviewed, edit reset to `DRAFT`,
Void, route/browser rendering, and unchanged warning-only release behavior.
...
Prompt 77 should update the FlightLeg detail Release Evidence Action Panel
dispatch card to show dispatch package review status and freshness while
keeping release readiness and release actions warning-only.

Implementation status: complete.

Prompt 77 updated the FlightLeg detail Release Evidence Action Panel dispatch
card to show dispatch package review state and ready/reviewed/voided timestamp
context. `VOIDED` packages surface as needing attention in the panel. Release
...
Prompt 80 refactored release-readiness snapshot creation into a shared helper.
Explicit preview snapshots still work, and release-control actions now attempt
pre-action release-attempt snapshots plus linked release audit events. Release
actions remain warning-only.
